...
//    포인트 사용
    public UserPoint use(long id, long point) {
        ...
        long newPoint = originPoint - point;
        if (originPoint < point) {
            throw new IllegalArgumentException("포인트가 부족합니다.");
        }
        UserPoint updatedUser = userPointTable.insertOrUpdate(id, newPoint);
        historyInsert(updatedUser.id(), updatedUser.point(), TransactionType.USE, point);
        return updatedUser;
    }
...
//    포인트 충전
    public UserPoint charge(long id, long point) {
        ...
        UserPoint updatedUser = userPointTable.insertOrUpdate(id, newPoint);
        historyInsert(updatedUser.id(), updatedUser.point(), TransactionType.CHARGE, point);
        return updatedUser;
    }
